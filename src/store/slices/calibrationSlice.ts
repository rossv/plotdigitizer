...
export const createCalibrationSlice: StoreSlice<CalibrationSlice> = (set) => ({
    ...
    toggleXAxisLog: () => set(state => updateActiveWorkspace(state, (ws) => {
        ...
        if (newXAxis.p1 && newXAxis.p2) {
            try {
                ...
                newHistory.push({ series: updatedSeries, yAxes: ws.yAxes, xAxis: newXAxis, imageRotation: ws.imageRotation, description: 'Toggle X Log' });
    ...
    toggleYAxisLog: (axisId) => set(state => updateActiveWorkspace(state, (ws) => {
        ...
        const newHistory = ws.history ? ws.history.slice(0, ws.historyIndex + 1) : [];
        newHistory.push({ series: updatedSeries, yAxes: updatedYAxes, xAxis: ws.xAxis, imageRotation: ws.imageRotation, description: 'Toggle Y Log' });
        ...
        if (p.axis === 'X') {
            ...
            if (p.step === 2 && newAxis.p1 && newAxis.p2) {
                try {
                    ...
                    const newHistory = ws.history ? ws.history.slice(0, ws.historyIndex + 1) : [];
                    newHistory.push({ series: updatedSeries, yAxes: ws.yAxes, xAxis: newAxis, imageRotation: ws.imageRotation, description: 'Calibrate X' });
            ...
            if (isComplete && activeAxis) {
                ...
                const newHistory = ws.history ? ws.history.slice(0, ws.historyIndex + 1) : [];
                newHistory.push({ series: updatedSeries, yAxes: updatedYAxes, xAxis: ws.xAxis, imageRotation: ws.imageRotation, description: 'Calibrate Y' });
